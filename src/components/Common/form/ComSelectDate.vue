<!--
增加属性：
autoCalculateBtn: Boolean, // 是否自動帶入日期計算功能
calDateVal: {
  type: [Number, String],
}, // 計算天數(目前只考慮月份)  
autoToday: Boolean //自動帶入今日日期模式
-->
    ...
    >
      ...
      >
        ...
        <template v-slot:AutoCalculateBtn>
          <!-- <div v-if="autoCalculateBtn" class="row items-center justify-end">
            <q-btn
              v-close-popup
              label="確認"
              color="primary"
...
export default {
  ...
  data() {
    return {
      ...
      date: "",
      modified: false,
      autoTodayChanged:false, //紀錄是否自動帶入的日期已改變。
    };
  },
  created() {
  ...
  computed: {
    dateModel() {
      if (this.autoToday && !this.autoTodayChanged) {
        //自動帶入今日日期模式
        this.date = new Date();
        return this.date.toISOString().substring(0, 10);  // 順便轉成字串傳給child component
      }else{
        return this.date;
      ...
      if (this.range) {
        ...
        if (!newVal) {
          ...
          if(!this.autoTodayChanged){
            // 自動帶入今日日期模式
            // console.log("自動帶入今日日期模式>>");
            input = newVal.toISOString().substring(0, 10);
          }
          else{
            // 改變自動帶入今日日期
            // console.log("改變!!!!自動帶入今日日期模式>>",newVal);
            input = newVal;
          }
    ...
    "$attrs.value": {
      handler(val, old) {
        if (!val) {
          ...
          (typeof val === "string" && !this.autoTodayChanged)
        ){
          ...
          (typeof val === "string" && !this.autoCalculateBtn)
        ) {
  ...
  methods: {
    addOptionDate() {
      ...
      // const fromDate = new Date(this.date);
      // // 設定天數
      // // fromDate.addDays(10).toISOString().substring(0, 10);
      // //
      // obj.push(
      //   fromDate
      //     .addMonths(this.calDateVal)
